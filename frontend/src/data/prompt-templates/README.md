...
The upstream repository is tracked as a Git submodule at:

```text
third_party/awesome-gpt-image-2-prompts/
```

Update the submodule and sync the frontend JSON/license with:

```bash
npm run update:prompt-templates
```

If the submodule is already up to date and you only want to copy the current snapshot into the frontend bundle:
